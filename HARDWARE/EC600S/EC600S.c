...
void  MQTT_Init(void)
{
    ...
    delay_ms(500);
    strx=strstr((const char*)RxBuffer2,(const char*)"+QMTSUB: 0,1,0,0");//订阅成功
    while(strx==NULL)
    {
        strx=strstr((const char*)RxBuffer2,(const char*)"+QMTSUB: 0,1,0,0");//订阅成功
    }
...
void Send_SMS(char *phone,char *data)//发送短信
{
    ...
    delay_ms(500);
    printf (data);//文本内容
    Clear_Buffer2();	//清除上面的内容
    while((USART2->SR&0X40)==0);//
    USART2->DR = 0X1A;   //发送结束符
    delay_ms(500);
    while(1)
    {
        strx=strstr((const char*)RxBuffer2,(const char*)"+CMGS");//返回CMGS表明短信发送成功
        if(strx)
            break;//跳出
    }
    Clear_Buffer2();	//清除上面的内容
